...
export function start_one_chat(trans_words, yield_call) {
  ...
  return do_request(start_one_chat_url, JSON.stringify({
    "messages": [
      {
        "role": "user",
    ...
    if (responseBody.error_code == 110 && !yield_call) {
      ...
      // 再尝试调用一次
      var final_result = start_one_chat(trans_words, true)
...
// 获取AccessToken
export function get_oh_access_token(flush) {
  ...
  if (stored_access_token && !flush) {
    ...
    return do_request(access_token_url).then(responseBody => {
...
var OhTrans = new SubProcess({
  ...
  runnable: async (str)=> {
    ...
    var result = await start_one_chat(content);
    return result
  },
